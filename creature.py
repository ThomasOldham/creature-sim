...
@timer_decorator('creature.private_features')
...
@timer_decorator('creature.public_features')
...
def action_features(creature_storage: CreatureStorage, out: Optional[np.ndarray] = None) -> np.ndarray:
    if out is None:
        out = np.zeros((creature_storage.used_row_count(), ACTION_KINDS_COUNT), dtype=np.float64)
    if not out.size:
        return out
    for i in range(ACTION_KINDS_COUNT):
        mask = creature_storage.stats[:, LAST_ACTION] == i
        out[mask, i] = 1.0
    return out

DEFAULT_PUBLIC_FEATURES = np.zeros(NUM_PUBLIC_FEATURES, dtype=np.float64)

@timer_decorator('creature.transform_features')
...
@timer_decorator('creature.decide_action_kind')
...
@timer_decorator('creature._normalize_action_probabilities')
